...
function reducer(state, action) {
  ...
  if (action.type === 'create_game') {
    ...
    if (isValidPosition) {
      ...
      newSnake.shift();
      if (
        head.positionX === newState.apple.positionX &&
        head.positionY === newState.apple.positionY
      ) {
...
function validateSnakeHead(snake, head) {
  ...
  if (head.positionX >= 1 && head.positionX <= 30 && head.positionY >= 1 && head.positionY <= 51) {
    return true;
  }
...
export default function Game() {
  ...
  const {isStarted, snake, apple, direction, speed, currentScore, gameOver, gameWon, paused} =
    state;
  ...
  return (
    <>
      <div className='hidden lg:flex h-[500px] w-[510px] bg-custom-gradient backdrop-blur-md relative items-center justify-center shrink-0 p-6 rounded-sm'>
        ...
        <div className='h-full w-full flex justify-between'>
          <div className='bg-[#011627D6] h-full w-[238px] overflow-hidden rounded-md relative'>
            {(gameOver || gameWon || paused) && (
              <div className='w-full h-12 z-20 bg-black/30 text-xl absolute top-2/3 box-center'>
                <p
                  className={`${
                    (gameOver && 'text-secondary') || ((gameWon || paused) && 'text-primary')
                  }`}
                >
          ...
          <div className='flex flex-col gap-7 self-center w-[181.38214px]'>
...
// 			onClick={onClick}
// 			disabled={disabled}
// 			className={`flex justify-center items-center px-5 py-3 rounded-lg border border-borderActive text-borderActive
// 			hover:bg-borderActive hover:text-black transition-colors ${className}
// 		`}>
// 			{children}
// 		</button>
// 	);
...
const Dot = ({score, index}) => {
  ...
  // const color = !eaten ? 'bg-primary' : 'bg-secondary';
  return (
